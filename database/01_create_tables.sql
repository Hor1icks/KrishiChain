...
CREATE TABLE FARM (
  ...
  VerificationStatus      VARCHAR2(10 CHAR) DEFAULT 'PENDING' NOT NULL,
  VerificationRequestedAt DATE DEFAULT SYSDATE NOT NULL,
  ...
  CONSTRAINT FK_FARM_VERIFIER FOREIGN KEY (VerifiedByAdminID) REFERENCES ADMIN_STAFF (AdminID),
  CONSTRAINT CK_FARM_AREA CHECK (Area > 0),
  CONSTRAINT CK_FARM_STATUS CHECK (Status IN ('ACTIVE','INACTIVE')),
  CONSTRAINT CK_FARM_VERIFICATION CHECK (VerificationStatus IN ('PENDING','VERIFIED','REJECTED')),
  CONSTRAINT CK_FARM_VERIFICATION_SHAPE CHECK (
    (VerificationStatus = 'VERIFIED' AND VerificationReviewedAt IS NOT NULL AND VerifiedByAdminID IS NOT NULL) OR
...
CREATE TABLE STORAGE_UNIT (
  ...
  LocationTag  VARCHAR2(120 CHAR) NOT NULL,
  Capacity     NUMBER(12,3)      NOT NULL,
  ...
  CONSTRAINT UQ_UNIT_LOCATION UNIQUE (WarehouseID, LocationTag),
...
CREATE TABLE TRANSPORT_REQUEST (
  ...
  RequestType       VARCHAR2(20 CHAR) DEFAULT 'SALE_DELIVERY' NOT NULL,
  SaleOrderID       NUMBER(10),
  AllocationID      NUMBER(10),
  ...
  CONSTRAINT FK_TRANSPORT_ALLOCATION FOREIGN KEY (AllocationID) REFERENCES STORES (AllocationID) ON DELETE CASCADE,
  CONSTRAINT UQ_TRANSPORT_ORDER UNIQUE (SaleOrderID),
  CONSTRAINT UQ_TRANSPORT_ALLOCATION UNIQUE (AllocationID),
  CONSTRAINT CK_TRANSPORT_TYPE CHECK (RequestType IN ('SALE_DELIVERY','STORAGE_INBOUND')),
  CONSTRAINT CK_TRANSPORT_SHAPE CHECK (
    (RequestType = 'SALE_DELIVERY' AND SaleOrderID IS NOT NULL AND AllocationID IS NULL) OR
    (RequestType = 'STORAGE_INBOUND' AND AllocationID IS NOT NULL)),
